    ...
    <div className="grid gap-4">
      ...
      >
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-1 text-lg font-extrabold text-white">{Number(stats?.streak || 0)} día(s)</div>
            <div className="mt-1 text-sm text-slate-400">Último: {stats?.lastDay || '—'}</div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="text-xs font-semibold tracking-[0.22em] text-slate-500">APERTURAS</div>
            <div className="mt-1 text-lg font-extrabold text-white">{Number(stats?.opens || 0)}</div>
            <div className="mt-1 text-sm text-slate-400">Total en este dispositivo</div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="text-xs font-semibold tracking-[0.22em] text-slate-500">INBOX</div>
            <div className="mt-1 text-lg font-extrabold text-white">{Number(unreadCount || 0)} sin leer</div>
            <div className="mt-1 text-sm text-slate-400">Resultados y guardados</div>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {lastRecent ? (
            <Link
              ...
              className="inline-flex h-12 items-center justify-center rounded-2xl bg-cyan-400 px-5 text-base font-extrabold text-slate-950 transition active:scale-[0.99] hover:bg-cyan-300"
            >
              Continuar
            </Link>
          ) : (
          ...
          <Link
            to="/biblioteca"
            className="inline-flex h-12 items-center justify-center rounded-2xl bg-white/10 px-5 text-base font-extrabold text-white transition active:scale-[0.99] hover:bg-white/15"
          >
            Biblioteca
          </Link>
          <Link
            to="/quiz"
            className="inline-flex h-12 items-center justify-center rounded-2xl bg-emerald-500 px-5 text-base font-extrabold text-emerald-950 transition active:scale-[0.99] hover:bg-emerald-400"
          >
            Quiz
          </Link>
          <Link
        ...
        <section className="rounded-3xl border border-slate-800/60 bg-slate-900/20 p-5">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {recents.slice(0, 4).map((s) => (
              ...
              >
                <div className="text-sm font-extrabold text-white line-clamp-2">{s.title}</div>
                <div className="mt-1 text-xs font-semibold text-slate-400 line-clamp-1">{s.topic || 'Sin tema'}</div>
              </Link>
            ))}
          </div>
        </section>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-3xl border border-slate-800/60 bg-slate-900/20 p-5">
          ...
          <div className="mt-1 text-base font-extrabold text-white">Estudio</div>
          <div className="mt-2 text-sm text-slate-300">Explora la lámina, usa zoom fluido y revisa etiquetas.</div>
        </div>
        <div className="rounded-3xl border border-slate-800/60 bg-slate-900/20 p-5">
          <div className="text-xs font-semibold tracking-[0.22em] text-slate-500">MODO</div>
          <div className="mt-1 text-base font-extrabold text-white">Identificar Punto</div>
          <div className="mt-2 text-sm text-slate-300">Entrena reconocimiento tocando el hotspot correcto.</div>
        </div>
        <div className="rounded-3xl border border-slate-800/60 bg-slate-900/20 p-5">
          <div className="text-xs font-semibold tracking-[0.22em] text-slate-500">NUEVO</div>
          <div className="mt-1 text-base font-extrabold text-white">Medical Quiz 2026</div>
          <div className="mt-2 text-sm text-slate-300">Pin “?” + opciones A-D + feedback premium para memorizar rápido.</div>
        </div>
      </section>
    </div>
  )
